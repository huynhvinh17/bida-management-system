...
class BidaConfig:
    ...
    HISTORY_FILE = f"{DATA_FOLDER}/history.json"
...
# ================== QUẢN LÝ LỊCH SỬ THẮNG THUA ==================
class HistoryManager:
    def __init__(self):
        self.history_file = BidaConfig.HISTORY_FILE
    ...
    def add_match(self, winner_name, loser_name, winner_score, loser_score, table_name, date):
        match = {
            "date": date,
            "winner": winner_name,
            "loser": loser_name,
            ...
            "table": table_name,
            "month": datetime.datetime.fromisoformat(date).strftime("%Y-%m"),
            "day": datetime.datetime.fromisoformat(date).strftime("%d/%m/%Y %H:%M")
        }
        self.history.append(match)
        self.save_history()
        return True
    
    def get_monthly_ranking(self, year_month=None):
        ...
        for match in self.history:
            if match["month"] == year_month:
                wins[match["winner"]] += 1
                losses[match["loser"]] += 1
        
        all_players = set(wins.keys()) | set(losses.keys())
        ranking = []
        
        for player in all_players:
            win_count = wins.get(player, 0)
            loss_count = losses.get(player, 0)
        ...
        return ranking, year_month
    
    def get_all_months(self):
        months = set()
        for match in self.history:
            months.add(match["month"])
        return sorted(list(months), reverse=True)
    
    def get_recent_matches(self, limit=30):
        recent = sorted(self.history, key=lambda x: x["date"], reverse=True)
        return recent[:limit]
...
def setup_page():
    ...
    .winner-row {
        background: linear-gradient(135deg, #1a5c1a 0%, #2e7d32 100%);
        border-radius: 10px;
        padding: 10px;
        margin: 5px 0;
    }
    .loser-row {
        background: linear-gradient(135deg, #5c1a1a 0%, #7d2e32 100%);
        border-radius: 10px;
        padding: 10px;
        margin: 5px 0;
    }
...
def display_ez_scoreboard(session, table_id, dm):
    ...
    with col1:
        ...
        with c1:
            if st.button("+1", key=f"p1_add1_{table_id}_{int(time.time())}"):
                dm.update_score(table_id, "player1", 1, True)
                st.rerun()
        with c2:
            if st.button("+5", key=f"p1_add5_{table_id}_{int(time.time())}"):
                dm.update_score(table_id, "player1", 5, True)
                st.rerun()
        with c3:
            if st.button("-1", key=f"p1_sub1_{table_id}_{int(time.time())}"):
                dm.update_score(table_id, "player1", 1, False)
                st.rerun()
        with c4:
            if st.button("-5", key=f"p1_sub5_{table_id}_{int(time.time())}"):
    ...
    with col2:
        ...
        if st.button("🔄 HOAN DOI", key=f"swap_btn_{table_id}_{int(time.time())}"):
    ...
    with col3:
        ...
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            if st.button("+1", key=f"p2_add1_{table_id}_{int(time.time())}"):
                dm.update_score(table_id, "player2", 1, True)
                st.rerun()
        with c2:
            if st.button("+5", key=f"p2_add5_{table_id}_{int(time.time())}"):
                dm.update_score(table_id, "player2", 5, True)
                st.rerun()
        with c3:
            if st.button("-1", key=f"p2_sub1_{table_id}_{int(time.time())}"):
                dm.update_score(table_id, "player2", 1, False)
                st.rerun()
        with c4:
            if st.button("-5", key=f"p2_sub5_{table_id}_{int(time.time())}"):
        ...
        for i in range(4):
            ...
            with cols[i]:
                ...
                if tid in active:
                    ...
                    """, unsafe_allow_html=True)
                    if st.button(f"📱 VAO BAN", key=f"view_{tid}_{int(time.time())}"):
                    ...
                    """, unsafe_allow_html=True)
                    if st.button(f"🎯 DAT BAN", key=f"book_{tid}_{int(time.time())}"):
...
def display_customer_booking_form(dm):
    if st.session_state.get("show_customer_booking", False):
        with st.expander("🎯 ĐẶT BÀN", expanded=True):
            ...
            with col1:
                p1 = st.text_input("🎯 Tên Player 1", key=f"bk_p1_{tid}")
                p2 = st.text_input("🎯 Tên Player 2", key=f"bk_p2_{tid}")
            with col2:
                st.markdown(f"""
            ...
            if st.button("✅ XÁC NHẬN", key=f"confirm_booking_{tid}"):
                if p1 and p2:
            ...
            if st.button("❌ HỦY", key=f"cancel_booking_{tid}"):
                st.session_state.show_customer_booking = False
                st.rerun()
...
def display_order_section(table_id, session, dm):
    ...
    if cat and item:
        ...
        if info['stock'] >= qty:
            if st.button(f"➕ THÊM MÓN", key=f"add_{table_id}_{int(time.time())}"):
    ...
    if orders:
        ...
        for idx, o in enumerate(orders):
            ...
            with c4:
                if st.button("🗑️", key=f"del_{table_id}_{idx}_{int(time.time())}"):
    ...
    if st.session_state.get("show_customer_detail", False) and st.session_state.get("customer_selected_table"):
        ...
        if tid in active:
            ...
            with tab3:
                ...
                if st.button("🔔 BÁO TÍNH TIỀN", key=f"bill_{tid}_{int(time.time())}"):
                    dm.add_alert(tid, BidaConfig.TABLES[tid]['name'])
                    st.success("✅ Đã gửi yêu cầu! Nhân viên sẽ đến ngay!")
                    st.balloons()
                    time.sleep(1)
            
            if st.button("🔙 QUAY LẠI", key=f"back_cust_{tid}"):
...
def display_owner_interface(dm):
    ...
    if alerts:
        ...
        for i, a in enumerate(alerts):
            ...
            with c2:
                if st.button(f"💰 TÍNH TIỀN", key=f"pay_alert_{i}_{int(time.time())}"):
                    st.session_state.owner_pay_table = a['table_id']
                    st.session_state.show_owner_payment = True
                    dm.remove_alert(i)
                    st.rerun()
            with c3:
                if st.button(f"✅ BỎ QUA", key=f"ignore_{i}_{int(time.time())}"):
                    dm.remove_alert(i)
                    st.rerun()
        st.markdown("---")
        ...
        for i in range(4):
            ...
            with cols[i]:
                ...
                if tid in active:
                    ...
                    """, unsafe_allow_html=True)
                    if st.button(f"💰 TIỀN", key=f"pay_{tid}_{int(time.time())}"):
    ...
    if st.session_state.get("show_owner_payment", False) and st.session_state.get("owner_pay_table"):
        ...
        if tid in active:
            ...
            with st.expander(f"💰 TÍNH TIỀN BÀN SỐ {tid}", expanded=True):
                ...
                **💵 TỔNG CỘNG:** {table_fee + food_total:,}đ
                """)
                if st.button(f"✅ XÁC NHẬN THANH TOÁN", key=f"confirm_pay_{tid}_{int(time.time())}"):
...
def display_stock_management(dm):
    ...
    col1, col2, col3 = st.columns([2, 1, 1])
    with col1:
        item = st.selectbox("Chọn sản phẩm", all_items, key="import_item")
    with col2:
        qty = st.number_input("Số lượng", min_value=1, max_value=1000, value=50, key="import_qty")
    with col3:
        if st.button("📦 NHẬP HÀNG", key="import_stock_btn"):
...
def display_monthly_ranking(dm):
    st.markdown("### 🏆 BẢNG XẾP HẠNG THÁNG 🏆")
    st.markdown("*Người chơi có số trận thắng nhiều nhất trong tháng*")
    
    months = dm.history_manager.get_all_months()
    
    if not months:
        st.info("📭 Chưa có dữ liệu trận đấu nào. Hãy kết thúc một trận để bắt đầu!")
        return
    
    selected_month = st.selectbox("📅 Chọn tháng", months, key="month_select")
    
    if selected_month:
        ranking, month_display = dm.history_manager.get_monthly_ranking(selected_month)
        
        if ranking:
            st.markdown("### 🥇🥈🥉 TOP 3 THÁNG")
            cols = st.columns(3)
            
            for i, r in enumerate(ranking[:3]):
                with cols[i]:
                    medal = ["🥇", "🥈", "🥉"][i]
                    bg_color = ["#ffd700", "#c0c0c0", "#cd7f32"][i]
                    st.markdown(f"""
                    <div style='background: {bg_color}20; border-radius: 15px; padding: 15px; text-align: center; border: 2px solid {bg_color};'>
                        <h1 style='font-size: 40px;'>{medal}</h1>
                        <h3>{r['player']}</h3>
                        <p>🏆 Thắng: <b>{r['wins']}</b> trận</p>
                        <p>📉 Thua: {r['losses']} trận</p>
                        <p>📊 Tỉ lệ thắng: <b>{r['win_rate']}%</b></p>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            df_display = df[["rank", "player", "wins", "losses", "total", "win_rate"]]
            df_display.columns = ["Hạng", "Người chơi", "🏆 Thắng", "📉 Thua", "📊 Tổng", "🎯 Tỉ lệ %"]
            
            st.dataframe(df_display, use_container_width=True, hide_index=True)
            
            st.markdown("### 📊 BIỂU ĐỒ THẮNG - THUA")
            fig = go.Figure(data=[
                go.Bar(name="Thắng", x=[r['player'] for r in ranking[:10]], y=[r['wins'] for r in ranking[:10]], marker_color="#2e7d32"),
                go.Bar(name="Thua", x=[r['player'] for r in ranking[:10]], y=[r['losses'] for r in ranking[:10]], marker_color="#c62828")
            ])
            fig.update_layout(barmode='group', title="Top 10 người chơi xuất sắc nhất tháng", height=400)
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info(f"📭 Tháng {selected_month} chưa có dữ liệu trận đấu nào!")

def display_match_history(dm):
    st.markdown("### 📜 LỊCH SỬ CÁC TRẬN ĐẤU 📜")
    
    recent_matches = dm.history_manager.get_recent_matches(limit=30)
    
    if not recent_matches:
        st.info("📭 Chưa có trận đấu nào được ghi nhận!")
        return
    
    for match in recent_matches:
        st.markdown(f"""
        <div class="winner-row">
            🏆 <b>{match['winner']}</b> {match['winner_score']} - {match['loser_score']} <b>{match['loser']}</b><br>
            🎱 {match['table']} | 📅 {match['day']}
        </div>
        """, unsafe_allow_html=True)
...
def main():
    ...
    if st.session_state.mode is None:
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("👤 KHACH HANG", key="mode_customer_main", use_container_width=True):
                st.session_state.mode = "customer"
                st.rerun()
        with c2:
            if st.button("👑 CHU QUAN", key="mode_owner_main", use_container_width=True):
                st.session_state.mode = "owner"
                st.rerun()
    else:
        if st.button("🔄 DOI CHE DO", key="change_mode_main", use_container_width=True):
            st.session_state.mode = None
            st.session_state.owner_authenticated = False
            st.rerun()
        ...
        if st.session_state.mode == "customer":
            ...
            if not st.session_state.owner_authenticated:
                ...
                pwd = st.text_input("Mat khau:", type="password", key="owner_pwd")
                if st.button("🔓 XAC NHAN", key="check_password_main"):
                ...
                tabs = st.tabs(["🎯 QUAN LY BAN", "🏆 XEP HANG THANG", "📜 LICH SU TRAN DAU", "📊 DOANH THU", "📦 QUAN LY KHO", "📝 QUAN LY MENU"])
                with tabs[0]:
                    display_owner_interface(dm)
                    display_owner_payment_form(dm)
                with tabs[1]:
                    display_monthly_ranking(dm)
                with tabs[2]:
                    display_match_history(dm)
                with tabs[3]:
...
if __name__ == "__main__":
    main()# bida_system_final_working.py
import streamlit as st
import pandas as pd
import datetime
import json

...
def display_ez_scoreboard(session, table_id, dm):
    ...
    if st.session_state[swap_key]:
        ...
        p1_border, p2_border = "#ffffff", "#ffd700"
        p1_icon, p2_icon = "⚪", "🟡"
    else:
        p1_color, p2_color = "#ffd700", "#ffffff"
        p1_bg, p2_bg = "#1a1a00", "#1a1a1a"
        p1_border, p2_border = "#ffd700", "#ffffff"
        p1_icon, p2_icon = "🟡", "⚪"
    ...
    with col1:
        st.markdown(f"""
        <div style='background: {p1_bg}; border-radius: 15px; padding: 15px; text-align: center; border: 2px solid {p1_border};'>
            ...
            <h1 style='font-size: 80px; color: {p1_color};'>{session['score']['player1']}</h1>
            <p style='color: {p1_color};'>Inn. {session['innings']['player1']}</p>
            <p style='color: #aaaaaa;'>HR {p1_hr} | Avg {p1_avg:.3f}</p>
        </div>
        """, unsafe_allow_html=True)
        
        st.markdown(f"<p style='color: {p1_color}; text-align: center;'>🎮 ĐIỀU KHIỂN</p>", unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(f"""
        <div style='background: #000000; border-radius: 15px; padding: 20px; text-align: center; border: 1px solid #333;'>
            <h1 style='font-size: 42px; color: #00ff88;'>{minutes:02d}:{seconds:02d}</h1>
            <p style='color: #aaaaaa;'>{BidaConfig.TABLES[table_id]['price_per_hour']:,}đ/giờ</p>
            <div>
                <span style='color: #ffd700;'>●</span> <span style='color: #ffffff;'>●</span>
            </div>
            <div style='margin-top: 10px;'>
                <button style='background:#333; border:none; border-radius:5px; padding:5px 10px; color:white; cursor:pointer;' onclick=''>🔄 HOÁN ĐỔI</button>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        st.markdown(f"""
        <div style='background: {p2_bg}; border-radius: 15px; padding: 15px; text-align: center; border: 2px solid {p2_border};'>
            <div style='display: flex; align-items: center; justify-content: center; gap: 8px;'>
            ...
            <h1 style='font-size: 80px; color: {p2_color};'>{session['score']['player2']}</h1>
            <p style='color: {p2_color};'>Inn. {session['innings']['player2']}</p>
            <p style='color: #aaaaaa;'>HR {p2_hr} | Avg {p2_avg:.3f}</p>
        </div>
        """, unsafe_allow_html=True)
        
        st.markdown(f"<p style='color: {p2_color}; text-align: center;'>🎮 ĐIỀU KHIỂN</p>", unsafe_allow_html=True)
        col_a, col_b, col_c, col_d = st.columns(4)
        with col_a:
        ...
        with st.expander("🎯 ĐẶT BÀN", expanded=True):
            ...
            if st.button("✅ XÁC NHẬN", type="primary"):
                if player1 and player2:
                    dm.start_session(table_id, player1, "customer", player1, player2)
...
def display_order_section(table_id, session, dm):
    ...
    with col1:
        category = st.selectbox("Danh mục", list(dm.menu_manager.menu.keys()), key=f"cat_{table_id}")
    with col2:
        if category:
            items = list(dm.menu_manager.menu[category].keys())
    ...
    if category and item:
        info = dm.menu_manager.menu[category][item]
        ...
        if info['stock'] < quantity:
            ...
            if st.button(f"➕ THÊM MÓN", key=f"add_{table_id}"):
                success, msg = dm.add_order(table_id, category, item, quantity, info['price'])
                if success:
                    st.success(msg)
                    time.sleep(0.5)
                    st.rerun()
...
def display_customer_table_detail(dm):
    if st.session_state.get("show_customer_detail", False) and st.session_state.get("customer_selected_table"):
        ...
        if table_id in active_sessions:
            ...
            with tab1:
                display_ez_scoreboard(session, table_id, dm)
            
            with tab2:
                display_order_section(table_id, session, dm)
            ...
            with tab3:
                ...
                if st.button("🔔 BÁO TÍNH TIỀN", type="primary", use_container_width=True):
                    dm.add_alert(table_id, BidaConfig.TABLES[table_id]['name'])
...
def display_owner_interface(dm):
    ...
    active_sessions = {s["table_id"]: s for s in dm.get_active_sessions()}
    pending_alerts = dm.get_pending_alerts()
            ...
            with st.expander(f"💰 TÍNH TIỀN BÀN SỐ {table_id}", expanded=True):
                ...
                **💵 TỔNG CỘNG:** {table_fee + food_total:,}đ
                ...
                if st.button(f"✅ XÁC NHẬN THANH TOÁN", type="primary"):
                    result = dm.end_session(table_id)
                    if result:
...
def display_stock_management(dm):
    ...
    stock_data = []
    for category, items in dm.menu_manager.menu.items():
    ...
    with col3:
        if st.button("📦 NHẬP HÀNG"):
            if dm.menu_manager.add_stock(restock_item, restock_qty):
...
def display_menu_management(dm):
    ...
    with tab1:
        ...
        col1, col2 = st.columns(2)
        with col1:
            category = st.selectbox("Chọn danh mục", list(dm.menu_manager.menu.keys()), key="add_cat")
        with col2:
        ...
        with col4:
            if st.button("➕ THÊM MÓN", key="add_item"):
                if new_item:
                    if dm.menu_manager.add_item(category, new_item, new_price, new_stock, new_unit):
    ...
    with tab2:
        ...
        col1, col2 = st.columns(2)
        with col1:
            edit_cat = st.selectbox("Chọn danh mục", list(dm.menu_manager.menu.keys()), key="edit_cat")
        with col2:
            if edit_cat:
                items = list(dm.menu_manager.menu[edit_cat].keys())
                edit_item = st.selectbox("Chọn món", items, key="edit_item")
        
        if edit_cat and edit_item:
            current = dm.menu_manager.menu[edit_cat][edit_item]
            col1, col2 = st.columns(2)
            with col1:
                new_price = st.number_input("Giá mới", min_value=1000, step=1000, value=current["price"], key="edit_price")
                if st.button("💾 CẬP NHẬT GIÁ"):
                    dm.menu_manager.update_item_price(edit_cat, edit_item, new_price)
            ...
            with col2:
                ...
                if st.button("📦 CẬP NHẬT TỒN KHO"):
                    dm.menu_manager.update_item_stock(edit_cat, edit_item, new_stock)
    ...
    with tab3:
        ...
        col1, col2 = st.columns(2)
        with col1:
            del_cat = st.selectbox("Chọn danh mục", list(dm.menu_manager.menu.keys()), key="del_cat")
        with col2:
            if del_cat:
                items = ["-- Chọn --"] + list(dm.menu_manager.menu[del_cat].keys())
        ...
        if del_cat and del_item and del_item != "-- Chọn --":
            if st.button("🗑️ XÓA MÓN", type="primary"):
                if dm.menu_manager.delete_item(del_cat, del_item):
...
def display_detailed_statistics(dm):
    ...
    total_table, total_food, table_revenue, food_revenue = dm.get_today_revenue()
